            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <th
                    key={header.id}
                    className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors duration-150"
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      {(() => {
                        ...
                        if (sortOrder === 'asc') return <FaArrowUp className="ml-1 text-emerald-600" size={12} />;
                        if (sortOrder === 'desc') return <FaArrowDown className="ml-1 text-emerald-600" size={12} />;
                        return null;
                      })()}
                    </div>
                  </th>
              ...
              table.getRowModel().rows.map((row) => (
                <tr key={row.id} className="hover:bg-gray-50 transition-colors duration-150">
                  {row.getVisibleCells().map((cell) => (
                    <td key={cell.id} className="px-5 py-4 whitespace-nowrap text-sm text-gray-700">
