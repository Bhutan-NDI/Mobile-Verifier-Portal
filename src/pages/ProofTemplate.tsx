...
const ProofTemplate = () => {
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    try {
      ...
      setTemplates(Array.isArray(result.data) && result.data.length > 1 ? result.data[1] : []);
    } catch (error) {
  ...
  const fetchRoles = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${base_api_url}/mobile-verifier/v1/verifier-role?pageSize=100&page=1`,
        {
          method: "GET",
          headers: {
            accept: "*/*",
  ...
  }, [isAuthenticated, pagination.page, pagination.pageSize, fetchRoles, fetchTemplates]);
